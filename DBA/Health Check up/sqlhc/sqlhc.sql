...
REM
REM $Header: 1366133.1 sqlhc.sql 15th Feb 2020 stelios.charalambides, carlos.sierra $
REM
...
-- (015)
-- 15th February 2020 -- Fixed issue with code where wrong observations was being shown. 
-- DBMS_STATS automatic gathering on 11g
INSERT INTO plan_table (id, operation, object_alias, other_tag, remarks, projection,position)
SELECT :E_GLOBAL, 'DBMS_STATS', SYSTIMESTAMP, 'DBA_AUTOTASK_CLIENT',
       CASE
         WHEN '^^is_ebs.' = 'Y' THEN 'Disable this job immediately and re-gather statistics for all affected schemas using FND_STATS or coe_stats.sql.'
         WHEN '^^is_siebel.' = 'Y' THEN 'Disable this job immediately and re-gather statistics for all affected schemas using coe_siebel_stats.sql.'
         WHEN '^^is_psft.' = 'Y' THEN 'Disable this job immediately and re-gather statistics for all affected schemas using pscbo_stats.sql.<br>'
         ELSE 'Be aware that small sample sizes could produce poor quality histograms,<br>'||CHR(10)||'which combined with bind sensitive predicates could render suboptimal plans.'
         END, 
	   CASE
         WHEN '^^is_ebs.' = 'Y' THEN 'See <a target="MOS" href="^^doc_link.156968.1">156968.1</a>.'     
         WHEN '^^is_siebel.' = 'Y' THEN 'See <a target="MOS" href="^^doc_link.781927.1">781927.1</a>.'
         WHEN '^^is_psft.' = 'Y' THEN 'See <a target="MOS" href="^^doc_link.1322888.1">1322888.1</a>.'
         ELSE 'See <a target="MOS" href="^^doc_link.465787.1">465787.1</a>.'
         END, 66
...
SELECT :E_GLOBAL, 'PLAN', SYSTIMESTAMP, 'OPTIMIZER_ENV',
       'Check the applicability of having multiple CBO environmets set up for one SQL. There are ('||COUNT(DISTINCT optimizer_env_hash_value)||' distinct CBO Environments for this one SQL). See https://docs.oracle.com/database/121/REFRN/GUID-2B9340D7-4AA8-4894-94C0-D5990F67BE75.htm#REFRN30246',
...
INSERT INTO plan_table (id, operation, object_alias, other_tag, remarks, projection,position)
SELECT :E_GLOBAL, 'PLAN', SYSTIMESTAMP, 'OPTIMIZER_ENV',
       'Check the applicability of having multiple CBO environmets set up for one SQL. (Currently refernces '||COUNT(DISTINCT optimizer_env_hash_value)||' distinct CBO Environments for this one SQL) See https://docs.oracle.com/database/121/REFRN/GUID-2B9340D7-4AA8-4894-94C0-D5990F67BE75.htm#REFRN30246',
       'Distinct CBO Environments may produce different Plans.', 71
  FROM gv$sql
 WHERE :health_checks = 'Y'
   AND sql_id = :sql_id
...
INSERT INTO plan_table (id, operation, object_alias, other_tag, remarks, projection,position)
SELECT :E_GLOBAL, 'PLAN', SYSTIMESTAMP, 'OPTIMIZER_ENV',
       'Check the applicability of having multiple CBO environmets set up for one SQL. (Currently references '||COUNT(DISTINCT optimizer_env_hash_value)||' distinct CBO Enviornments for this one SQL.) See https://docs.oracle.com/database/121/REFRN/GUID-2B9340D7-4AA8-4894-94C0-D5990F67BE75.htm#REFRN30246',
