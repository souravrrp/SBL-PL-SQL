select K.YEAR,
       ...
       round(((K.Active_Account - nvl(T.Arr_AC, 0) +
             nvl(T.Arr_not_paying, 0)) / K.Active_Account) * 100,
             2) as "PAYING"  
  FROM (SELECT h.SHOP_CODE,
         ...
         GROUP BY H.SHOP_CODE,
                  ...
                  h.YEAR,
                  h.PERIOD) K,
       (SELECT v1.SHOP_CODE, v1.Arr_AC, v2.Arr_not_paying
          from (SELECT SHOP_CODE, nvl(COUNT(*), 0) AS Arr_AC
                  FROM IFSAPP.hpnret_form249_arrears_tab h
                 WHERE H.YEAR = '&YEAR'
                   AND H.PERIOD = '&MONTH'
                   AND H.ARR_AMT > 0
               ...
               (SELECT SHOP_CODE, nvl(COUNT(*), 0) as Arr_not_paying
                  FROM IFSAPP.hpnret_form249_arrears_tab h
                 WHERE H.YEAR = '&YEAR'
                   AND H.PERIOD = '&MONTH'
                   AND H.ARR_AMT > 0
