        ...
        --h.actual_sales_date,
         ...
         case
           ...
           when (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
            ...
            '061-120'
         /*when (((last_day(to_date('&year_i'||'/'||'&period'||'/1', 'YYYY/MM/DD')) - h.sales_date) - ((h.total_ucc + h.amount_financed - h.act_out_bal) / h.monthly_pay) * 30) -30) > 120 
         and (((last_day(to_date('&year_i'||'/'||'&period'||'/1', 'YYYY/MM/DD')) - h.sales_date) - ((h.total_ucc + h.amount_financed - h.act_out_bal) / h.monthly_pay) * 30) -30) <= 365 then '121-365'*/
           else
            '120+'
         end band,
         H.ACT_OUT_BAL,
         ...
         where h.year = '&year_i'
           ...
           and h.act_out_bal > 0
           and h.shop_code not in ('JWSS', 'SAOS', 'SWSS', 'WSMO', 'SITM', 'WITM', 'SSAM') --Wholesale Sites
           and h.shop_code not in ('SAPM',
                                   'SCOM',
                                   'SCSM',
                                   'SESM',
                                   'SHOM',
