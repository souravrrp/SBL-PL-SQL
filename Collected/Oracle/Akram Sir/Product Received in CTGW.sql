...
select R.order_no,
       ...
       (IFSAPP.PURCHASE_ORDER_LINE_API.Get_Buy_Unit_Price(R.order_no,
                                                         R.line_no,
                                                         R.release_no) * R.qty_arrived) Total_Price,
 ...
 where IFSAPP.IDENTITY_INVOICE_INFO_API.Get_Group_Id('SBL',
   ...
   and R.vendor_no not in ('XLBD-SACF', 'XLBD-SRAV', 'XLBD-SRFR', 'XLBD-IAL')
   and R.contract = 'CTGW'
   /*and R.order_no like '&order_no'*/
   /*and substr(R.order_no, 1,2) like 'L-%'*/
   /*and R.part_no in ('SRSM-TABLE', 'SRSM- WOODEN COVER', 'SRSM-STAND-METAL-STEEL')*/
 order by 1, 2
