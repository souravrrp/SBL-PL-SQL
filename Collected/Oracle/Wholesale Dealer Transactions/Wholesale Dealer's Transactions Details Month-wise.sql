--*****Wholesale Dealer's Transactions Month-wise
...
select O.dealer_id,
       ...
       '' DELIVERY_SITE,
       '' "DESCRIPTION",
       to_date('&YEAR' || '/' || '&PERIOD' || '/1', 'YYYY/MM/DD') "DATE",
       '' INVOICE_NO,
       NVL(O.CLOSE_BALANCE, 0) "AMOUNT (IN TAKA)"
  from IFSAPP.SBL_DEALER_BALANCE O
 where O.YEAR = EXTRACT(YEAR FROM(to_date('&YEAR' || '/' || '&PERIOD' || '/1',
                                     'YYYY/MM/DD') - 1))
   AND O.PERIOD = EXTRACT(MONTH FROM(to_date('&YEAR' || '/' || '&PERIOD' || '/1',
                                       'YYYY/MM/DD') - 1))
