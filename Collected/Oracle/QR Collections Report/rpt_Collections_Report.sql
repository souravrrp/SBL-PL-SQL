...
select c.contract,
       ...
       (select sum(t.comm_calc_amount)
         ...
         where t.receipt_no = c.receipt_no
           and t.comm_calc_amount > 0) comm_calc_amount,
       c.payment_date,
       (select h.cash_conversion_on_date
          from ifsapp.hpnret_form249_arrears_tab h
         where h.acct_no = c.account_no
           and h.year = extract(year from(last_day(h.sales_date) + 1))
           and h.period = extract(month from(last_day(h.sales_date) + 1))) cash_conversion_on_date,
       c.pay_method,
       c.rowstate
 ...
 where c.payment_date between to_date('&from_date', 'YYYY/MM/DD') and
   ...
   and (select h.cash_conversion_on_date
          from ifsapp.hpnret_form249_arrears_tab h
         where h.acct_no = c.account_no
           and h.year = extract(year from(last_day(h.sales_date) + 1))
           and h.period = extract(month from(last_day(h.sales_date) + 1))) <=
       to_date('&to_date', 'YYYY/MM/DD')
