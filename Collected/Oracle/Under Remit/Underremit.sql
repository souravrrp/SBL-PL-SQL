...
SELECT 
       ...
       ifsapp.site_transaction_types_api.Get_Transaction_Type('SSL',bk.transaction_code) "Expense_Name", 
       nv.series_id "Invoice_Series",
       nv.invoice_no "Invoice_No",
      ...
      'Underremitted' "Status",
       ...
       substr(nv.notes,instr(nv.notes,'RSL REFERENCE')+16,14) "RSL Reference",
       substr(nv.notes,instr(nv.notes,'REMARKS')+9,70) "Remarks"
FROM   ifsapp.bcb_statement_details bk,
       ifsapp.invoice_ledger_item_cu_qry nv,
       ifsapp.bcb_statement bh
WHERE  bh.contract = '&Site'
AND    bk.voucher_date > to_date('2009/07/31','YYYY/MM/DD')
AND    bh.bcb_statement_id = bk.bcb_statement_id
AND    bk.under_remitted = 'TRUE'
AND    nv.open_amount > 0
AND    bk.invoice_id = nv.invoice_id
AND    nv.series_id = 'UR'
AND    bk.company = bh.company
AND    bk.company = 'SSL'
AND    bk.company = nv.company
...
SELECT 
       substr(cl.ledger_item_series_id,1,5) "Branch",
       ifsapp.site_api.Get_Description(substr(cl.ledger_item_series_id,1,5)) "Branch_Name",
       ...
       substr(nv.notes,instr(nv.notes,'REMARKS')+9,70) "Remarks"
FROM   ifsapp.check_ledger_item cl,
       ifsapp.invoice_ledger_item_cu_qry nv,
       ifsapp.payment_way w
WHERE  substr(cl.ledger_item_series_id,1,5) = '&Site'
AND    cl.voucher_date > to_date('2009/07/31','YYYY/MM/DD')
AND    cl.state = 'UnderRemitted'
AND    nv.series_id IN ('UR','CF','CCUR')
AND    nv.open_amount > 0
AND    cl.diff_inv_no = nv.invoice_no
AND    cl.diff_inv_series_id = nv.series_id
AND    cl.company = nv.company
AND    cl.company = 'SSL'
AND    w.way_id = cl.way_id
AND    w.company = cl.company
ORDER BY substr(cl.ledger_item_series_id,1,5),nv.payer_identity,cl.voucher_date
