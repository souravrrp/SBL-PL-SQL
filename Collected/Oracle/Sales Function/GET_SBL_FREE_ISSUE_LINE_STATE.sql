create or replace function GET_SBL_FREE_ISSUE_LINE_STATE(invoice_id_ in number,
                                                         item_id_    in number,
...
begin
  ...
  fetch get_acct_info
    into ord_no_, line_no_, rel_no_, catalog_no_, sales_price_, sales_date_;
  close get_acct_info;

  if catalog_no_ is not null then
    status_ := GET_SBL_ACCOUNT_STATUS(ord_no_,
                                      line_no_,
                                      rel_no_,
                                      catalog_no_,
                                      sales_price_,
