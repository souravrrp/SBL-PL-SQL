select v.shop_code,
       ...
       /*trunc((select min(t.receipt_date)
              ...
              where t.part_no = v.part_no
                and t.serial_no = v.serial_no)) mgt_receipt_date,
       trunc(sysdate) -
       trunc((select min(t.receipt_date)
               from INVENTORY_PART_IN_STOCK_NOPAL t
              where t.part_no = v.part_no
                and t.serial_no = v.serial_no)) mgt_age,
       case
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = v.part_no
                       and t.serial_no = v.serial_no)) <= 90 then
          '03'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = v.part_no
                       and t.serial_no = v.serial_no)) <= 180 then
          '06'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = v.part_no
                       and t.serial_no = v.serial_no)) <= 270 then
          '09'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = v.part_no
                       and t.serial_no = v.serial_no)) <= 360 then
          '12'
         else
          '12+'
       end mgt_age_month,*/
