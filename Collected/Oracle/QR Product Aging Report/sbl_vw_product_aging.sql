...
select i.contract shop_code,
       ...
       /*trunc(i.receipt_date) shop_receipt_date,
       ...
       trunc((select min(t.receipt_date)
               from INVENTORY_PART_IN_STOCK_NOPAL t
              where t.part_no = i.part_no
                and t.serial_no = i.serial_no)) mgt_age,
       case
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = i.part_no
                       and t.serial_no = i.serial_no)) <= 90 then
          '03'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = i.part_no
                       and t.serial_no = i.serial_no)) <= 180 then
          '06'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = i.part_no
                       and t.serial_no = i.serial_no)) <= 270 then
          '09'
         when trunc(sysdate) -
              trunc((select min(t.receipt_date)
                      from INVENTORY_PART_IN_STOCK_NOPAL t
                     where t.part_no = i.part_no
                       and t.serial_no = i.serial_no)) <= 360 then
          '12'
         else
          '12+'
       end mgt_age_month,*/
       nvl((select s.sales_price
         ...
         where s.price_list_no = '1'
           ...
           and s.catalog_no = i.part_no), 0) NSP,
           ...
           and s.valid_from_date =
               (select max(p.valid_from_date)
                 ...
                 where p.price_list_no = '1'
                   and p.valid_from_date <= sysdate
                   and p.catalog_no = s.catalog_no)
           and s.catalog_no = i.part_no)), 0) VAT
