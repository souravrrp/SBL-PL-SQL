...
select p.order_no,
       ...
       p.qty_arrived,
       ifsapp.get_sbl_sales_price('3', p.part_no, trunc(p.arrival_date)) unit_nsp,
       (p.qty_arrived *
       ifsapp.get_sbl_sales_price('3', p.part_no, trunc(p.arrival_date))) total_nsp,
       trunc(p.arrival_date) arrival_date,
       p.state
  from PURCHASE_RECEIPT_NEW p
 where IFSAPP.INVENTORY_PART_API.Get_Part_Product_Family('SCOM', p.part_no) !=
       'RBOOK'
