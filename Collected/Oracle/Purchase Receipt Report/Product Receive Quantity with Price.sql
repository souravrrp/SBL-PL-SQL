...
select R.order_no,
 ...
 where IFSAPP.IDENTITY_INVOICE_INFO_API.Get_Group_Id('SBL',
                                                     R.vendor_no,
                                                     'Supplier') = '0'
   and IFSAPP.INVENTORY_PART_API.Get_Part_Product_Family(R.contract, R.part_no) not in
       ('RBOOK', 'GIFT VOUCHER')
   ...
   and R.vendor_no not in
       ('XLBD-SACF', 'XLBD-SRAV', 'XLBD-SRFR'/*, 'XLBD-IAL'*/) /*= 'XLBD-MASB'*/
   /*and R.contract in ('CTGW', 'SACF', 'SAVF')*/
   /*and R.order_no like '&order_no'*/
   /*and substr(R.order_no, 1, 2) not like 'F-%'*/
   /*and R.part_no \*in
       ('SRSM-TABLE', 'SRSM- WOODEN COVER', 'SRSM-STAND-METAL-STEEL')*\ \*= 'SRREF-SINGER-FF2-66D'*\ like 'SGTV-%'*/
   /*and IFSAPP.INVENTORY_PRODUCT_CODE_API.Get_Description(ifsapp.inventory_part_api.Get_Part_Product_Code('SCOM',
                                                                                                         R.part_no)) =
       'Beko'*/
   /*and ifsapp.INVENTORY_PRODUCT_FAMILY_API.Get_Description(ifsapp.inventory_part_api.Get_Part_Product_Family(R.contract,
                                                                                                             R.part_no))
       in ('WASHING-MACHINE-FRONT-LOADER', 'WASHING-MACHINE-TOP-LOADER', 'Washing Machine Semi Auto Top Load', 'Washing Machine/Washer')*/
