...
select x.year,
  ...
  from (select j.year, j.period, j.part_no, nvl(l.qtn, 0) qtn
          from (select extract(year from add_months(start_date, level - 1)) "YEAR",
                       ...
                       $P{PRODUCT} part_no
                  from (select $P{FROM_DATE} start_date, $P{TO_DATE} end_date
                          from dual)
                connect by level <=
                    ...
                    select extract(year from r.receipt_date) "YEAR",
                      ...
                      from (select i.part_no,
                                    (SELECT P.PRODUCT_FAMILY
                                      ...
                                      WHERE P.PRODUCT_CODE = i.part_no) PRODUCT_FAMILY,
                                    (SELECT P.BRAND
                                       FROM IFSAPP.SBL_JR_PRODUCT_DTL_INFO P
                                      WHERE P.PRODUCT_CODE = i.part_no) BRAND,
                                    i.serial_no,
                              ...
                              where i.part_no = $P{PRODUCT}
                                and (SELECT P.PRODUCT_FAMILY
                                       FROM IFSAPP.SBL_JR_PRODUCT_DTL_INFO P
                                      WHERE P.PRODUCT_CODE = i.part_no) =
                                    $P{PRODUCT_FAMILY_SGL}
                                and (SELECT P.BRAND
                                       FROM IFSAPP.SBL_JR_PRODUCT_DTL_INFO P
                                      WHERE P.PRODUCT_CODE = i.part_no) =
                                    $P{BRAND}
                                and i.vendor_no = $P{SUPPLIER}
                                and IFSAPP.PART_CATALOG_API.Get_Serial_Tracking_Code_Db(i.part_no) =
                                    'SERIAL TRACKING'
                                and IFSAPP.IDENTITY_INVOICE_INFO_API.Get_Group_Id('SBL',
                                                                                  i.vendor_no,
                                                                                  'Supplier') = '0'
                                and trunc(i.receipt_date) between $P{FROM_DATE} and
                                    $P{TO_DATE}) r
 ...
 (select g.year,
    ...
    from (select j.*, k.*, $P{PRODUCT} part_no
            from (select extract(year from add_months(start_date, level - 1)) "YEAR",
                         extract(month from add_months(start_date, level - 1)) period
                    from (select $P{FROM_DATE} start_date, $P{TO_DATE} end_date
                            from dual)
    ...
    left join ( --***** Service Summary
                ...
                from (select s.*,
                              ...
                              (SELECT P.PRODUCT_FAMILY
                                ...
                                WHERE P.PRODUCT_CODE =
                                      substr(s.mch_code,
                                             1,
                                             instr(s.mch_code, '-', -3, 1) - 1)) PRODUCT_FAMILY,
                              (SELECT P.BRAND
                                 FROM IFSAPP.SBL_JR_PRODUCT_DTL_INFO P
                                WHERE P.PRODUCT_CODE =
                                      substr(s.mch_code,
                                             1,
                                             instr(s.mch_code, '-', -3, 1) - 1)) BRAND,
                              substr(s.mch_code,
     ...
     and g.service_month = m.service_month) y
    on x.year = y.year
