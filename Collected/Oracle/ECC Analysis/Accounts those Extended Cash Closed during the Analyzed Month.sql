--Accounts those Extended Cash Closed during the Analyzed Month (ECC Reversal)
SELECT H1.YEAR,
       H1.PERIOD,
       H1.SHOP_CODE,
       H1.ORIGINAL_ACCT_NO,
  ...
  FROM IFSAPP.HPNRET_FORM249_ARREARS_TAB H1, --Previous Month of the Analyzed Month
       (select t.account_no, ----List of Accounts Extended Cash Closed during the Analyzed Month
         ...
         where t.utilized = 1
           ...
           and trunc(ifsapp.hpnret_hp_head_api.Get_Closed_Date(t.account_no,
               ...
               LAST_DAY(TO_DATE('&YEAR_I' || '/' || '&PERIOD' || '/1',
                                'YYYY/MM/DD'))
           and t.from_date <= LAST_DAY(TO_DATE('&YEAR_I' || '/' || '&PERIOD' || '/1',
                                               'YYYY/MM/DD'))
           and t.to_date >=
               TO_DATE('&YEAR_I' || '/' || '&PERIOD' || '/1', 'YYYY/MM/DD')) C
