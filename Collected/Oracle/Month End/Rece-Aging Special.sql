select v1.band,
  ...
  from (select h.sales_date,
               ...
               case
                 when (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                      ...
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30)) - 30) <= 90 then
                  '000-090'
                 when (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30) - 30)) > 90 and
                      (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30) - 30)) <= 180 then
                  '091-180'
                 when (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30) - 30)) > 180 and
                      (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30) - 30)) <= 270 then
                  '181-270'
                 when (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
                               h.act_out_bal) / h.monthly_pay) * 30) - 30)) > 270 and
                      (((last_day(to_date('&year_i' || '/' || '&period' || '/1',
                                          'YYYY/MM/DD')) - h.sales_date) -
                      (round((h.total_ucc + h.amount_financed -
         ...
         where h.year = '&year_i'
           and h.period = '&period'
           /*and h.acct_no != 'FLP-H2816'*/ --'SES-H320'
           and h.act_out_bal > 0
           and h.shop_code not in ('JWSS', 'SAOS', 'SWSS', 'WSMO', 'SITM') --Wholesale Sites
