...
where
  ...
  trunc(c.ROWVERSION)  between to_date('&from_Date','dd/mm/yyyy') and to_date('&to_Date','dd/mm/yyyy') and 
  cust_ord_customer_api.get_cust_grp(t.CUSTOMER_NO) like '003' and 
  ...
  --UPPER(t.ORDER_NO) like UPPER('%$ORDER_NO%') and 
  --UPPER(t.CUSTOMER_NO) like UPPER('%$Customer_No%') and 
  --UPPER(customer_info_api.Get_Name(t.CUSTOMER_NO)) like UPPER('%$Customer_Name%') and 
  --UPPER(c.CATALOG_NO) like UPPER('%$PRODUCT_CODE%') and 
  --UPPER(c.VENDOR_NO) like UPPER('%$VENDOR_NO%') and 
  t.ORDER_NO = 'SCS-H20'/*in
  (
    select
        L.DEMAND_ORDER_NO
