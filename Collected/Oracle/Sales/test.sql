...
where --r.result_key = 3000519
  ...
  --r.s1 is not null
group by r.result_key, r.s1
  

--Distinct Sale date in a result key
select
  distinct(to_char(to_date(r.s1,'YYYY/MM/DD'),'YYYY-MM-DD')) as sale_date
from ifsapp.info_services_rpt r 
where r.s1 is not null and
  r.result_key = 2974040/*(select max(a.RESULT_KEY) 
                      from IFSAPP.ARCHIVE a where a.REPORT_TITLE='Sales Analysis Report')*/


--Status in the Customer Order Line
Select  * from customer_order_line d where
--d.state = 'Invoiced/Closed' and 
--d.rel_no=1 and
--d.contract = 'DGNB' AND
d.order_no= 'HBJ-H2443' --AND
--d.real_ship_date between to_date('2014/1/1','YYYY/MM/DD') and to_date('2014/1/31','YYYY/MM/DD')

select to_char(trunc(d.wanted_delivery_date), 'YYYY/MM/DD') from customer_order_line d

select Customer_Order_Line_API.Get_Total_Tax_Amount(c.order_no,c.line_no,c.rel_no,c.line_item_no) tax_amount,
customer_order_line_api.Get_Sale_Price_Total(c.order_no, c.LINE_NO, c.REL_NO ,c.LINE_ITEM_NO) NSP,
CUSTOMER_ORDER_LINE_API.Get_Real_Ship_Date(c.order_no, c.LINE_NO, c.REL_NO ,c.LINE_ITEM_NO) Real_Ship_Date,
CUSTOMER_ORDER_LINE_API.Get_Wanted_Delivery_Date(c.order_no, c.LINE_NO, c.REL_NO ,c.LINE_ITEM_NO) Wanted_Delivery_Date
...
Select  * from Customer_Order_Tab C
WHERE C.ORDER_NO = 'DMK-H2389'

--No. of Hire Accounts
Select  count(*) from HPNRET_HP_HEAD_TAB h where
--d.state = 'Invoiced/Closed' and 
--d.rel_no=1 and
h.contract = 'DGNB' AND
--h.account_no= 'DGN-H4218' AND
h.original_sales_date between to_date('2014/1/1','YYYY/MM/DD') and to_date('2014/2/1','YYYY/MM/DD')
