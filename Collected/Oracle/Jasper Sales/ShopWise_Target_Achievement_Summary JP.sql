SELECT MM.AREA_CODE,
  ...
  FROM (SELECT AA.SHOP_CODE, SUM(AA.REVENUE) TARGET_REVENUE
         ...
         WHERE AA.PERIOD BETWEEN $P{START_PERIOD} AND $P{END_PERIOD}
           AND AA.YEAR = $P{YEAR_I}
  ...
  LEFT JOIN (SELECT BB.AREA_CODE,
              ...
              WHERE EXTRACT(MONTH FROM BB.SALES_DATE) BETWEEN
                ...
                AND EXTRACT(YEAR FROM BB.SALES_DATE) = $P{YEAR_I}
 ...
 WHERE $X{IN, MM.AREA_CODE, AREA_CODE}
