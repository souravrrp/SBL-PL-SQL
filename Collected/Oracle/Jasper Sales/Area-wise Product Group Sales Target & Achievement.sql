--***** Area-wise Product Group Sales Target & Achievement
SELECT LL.AREA_CODE,
       /*LL.DISTRICT_CODE,
       LL.SHOP_CODE,*/
       LL.PRODUCT_GROUP,
       SUM(LL.TARGET_QTY) TARGET_QTY,
       SUM(LL.ACHIEV_QTY) ACHIEV_QTY,
       /*ROUND(CASE
       ...
       END,
       2) QTN_ACHIEV_PRCNTG,*/
       SUM(LL.TARGET_AMOUNT) TARGET_AMOUNT,
       SUM(LL.ACHIEV_AMOUNT) ACHIEV_AMOUNT --,
/*ROUND(CASE
...
2) AMT_ACHIEV_PRCNTG*/
  FROM (SELECT FF.AREA_CODE,
          ...
          FROM (SELECT SDI.AREA_CODE,
                 ...
                 WHERE TT.YEAR = '&YEAR_I'
                   AND TT.SHOP_CODE = SDI.SHOP_CODE
                   AND TT.PERIOD BETWEEN '&START_PERIOD' AND '&END_PERIOD'
          ...
          LEFT JOIN (SELECT CC.SHOP_CODE,
                     ...
                     WHERE DI.PRODUCT_CODE = CC.PRODUCT_CODE
                       ...
                       AND PPM.YEAR = '&YEAR_I'
                       AND EXTRACT(YEAR FROM CC.SALES_DATE) = '&YEAR_I'
 ...
 WHERE LL.PRODUCT_GROUP = /*'REFRIGERATOR'*/
       'FURNITURE' /*'SMALL APPLIANCE'*/
