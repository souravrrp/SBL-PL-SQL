select t.commission_receiver,
       ...
       /*(select t1.installment_amount
         ...
         where t1.transaction_id = t.org_transaction_id
           ...
           and rownum <= 1) org_installment_amount,*/
       t.comm_calc_amount,
...
--count(*)
 ...
 WHERE t.commission_sales_type = 'HP'
   and t.collection_type is null --= 'INST'
      --and t.approved_amount < 0 
   and t.site like '&Site'
   and trunc(t.calculated_date) BETWEEN to_date('&FromDate', 'YYYY/MM/DD') AND
       to_date('&ToDate', 'YYYY/MM/DD')
      --and t.calculated_date > t.approved_date 
      --and t.calculated_date < t.approved_date 
   and t.calculated_date is not null
      --and t.approved_date is null
   /*and t.account_no IN
       (SELECT DISTINCT t.account_no
          FROM hpnret_hp_dtl_tab t
         WHERE trunc(t.variated_date) BETWEEN
               to_date('&FromDate', 'yyyy/mm/dd') AND
               to_date('&ToDate', 'yyyy/mm/dd')
           AND t.contract like '&Site'
           AND t.rowstate in ('CashConverted', 'ExchangedIn', 'Returned'))*/
--AND t.commission_receiver like '&Salesman' and
--and t.order_no = 'DUT-H5509' 
--and t.receipt_no = '&receipt_no'  
--and t.approved_date BETWEEN to_date('&FromDate','YYYY/MM/DD') AND to_date('&ToDate','YYYY/MM/DD') 
--and t.transaction_id ='7156415' 
