...
where
  ...
  and trunc(t.WANTED_DELIVERY_DATE) between to_date('&fromDate','YYYY/MM/DD') and to_date('&toDate','YYYY/MM/DD')
  ...
  and c.CONTRACT in (select shop_code from shop_dts_info)
...
select
    ...
    (SELECT 
      ...
      FROM 
        cust_order_line_discount t 
      WHERE 
        t.order_no = c.Account_No AND 
        t.line_no = c.REF_LINE_NO AND 
        t.rel_no =  c.REF_REL_No AND 
        t.line_item_no = c.REF_LINE_ITEM_NO ) DISCOUNT,
...
from
  hpnret_hp_dtl_tab c
where
  trunc(c.VARIATED_DATE) between to_date('&fromDate','YYYY/MM/DD') and to_date('&toDate','YYYY/MM/DD')
...
select 
    ...
    (SELECT 
      ...
      FROM 
        cust_order_line_discount t 
      WHERE 
        t.order_no = c.order_no AND 
        t.line_no = c.LINE_NO AND 
        t.rel_no =  c.REL_No AND 
        t.line_item_no = c.LINE_ITEM_NO) DISCOUNT,
...
where
  trunc(c.DATE_RETURNED) between to_date('&fromDate','YYYY/MM/DD') and to_date('&toDate','YYYY/MM/DD') and 
  c.ROWSTATE  in ('ReturnCompleted') AND 
  SUBSTR(C.ORDER_NO,5,1) = 'R' and 
