...
where
  ...
  trunc(T.sales_date) >= to_date('2014/1/1', 'YYYY/MM/DD') and  
  ...
  h.rowstate not in ('AssumeClosed', 'Cancelled', 'CashConverted', 'DiscountClosed', 'ExchangedIn', 'Returned', 
    'RevertReversed', 'Reverted', 'TermChanged', 'ToBeReverted') and
  --h.part_no like '%FUR%'  and
  rownum <= 100
