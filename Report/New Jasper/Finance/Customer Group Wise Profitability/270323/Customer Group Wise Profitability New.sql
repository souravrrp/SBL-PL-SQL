/* Formatted on 3/27/2023 8:55:26 AM (QP5 v5.381) */
  SELECT CUSTOMER_NO,
         CUSTOMER_NAME,
         ...
         ROUND (SUM (dealer_incentive), 2)
             dealer_incentive,
         (SUM (discounted_nsp) - ROUND (SUM (dealer_incentive), 2))
             NET_SALES_AFTER_INCENTIVE,
         SUM (total_cost)
             total_cost,
           ((SUM (discounted_nsp) - ROUND (SUM (dealer_incentive), 2)) * 15)
         / 100
             OPEX_AMOUNT,
         (  (SUM (discounted_nsp) - ROUND (SUM (dealer_incentive), 2))
          - SUM (total_cost))
    ...
    FROM (SELECT t.invoice_id,
          ...
          SELECT t.invoice_id,
                 ...
                 (CASE
                      WHEN t.identity LIKE 'IT0%'
                      THEN
                          (l.part_price * l.qty_invoiced) * 0.01
                      ELSE
                          (l.part_price * l.qty_invoiced) * 0.032
                  END)
                     dealer_incentive
