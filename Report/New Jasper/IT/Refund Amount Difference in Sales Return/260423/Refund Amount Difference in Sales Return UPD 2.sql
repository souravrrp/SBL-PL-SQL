/* Formatted on 4/26/2023 4:21:00 PM (QP5 v5.381) */
  SELECT contract,
         customer_no,
         customer_name,
         phone_number,
    ...
    FROM (  SELECT colt.contract,
                   ...
                   (CASE
                    ...
                    END)
                       total_amount_settled,
                   ROUND (
                         (SUM (hsrlt.qty_returned_inv * hsrlt.sale_unit_price))
                       + (  SUM (
...
GROUP BY contract,
         ...
         line_amount,
         total_amount_settled
  HAVING total_amount_settled - SUM (refunded_amount) > 1 and line_amount - SUM (refunded_amount) > 1;