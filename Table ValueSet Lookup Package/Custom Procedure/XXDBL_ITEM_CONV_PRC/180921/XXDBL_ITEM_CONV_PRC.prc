/* Formatted on 9/18/2021 3:05:22 PM (QP5 v5.354) */
...
IS
    ...
    TYPE item_cat_interface_typ
        IS TABLE OF mtl_item_categories_interface%ROWTYPE
        INDEX BY BINARY_INTEGER;

    l_item_int_tbl       item_interface_typ;
    l_category_int_tbl   item_cat_interface_typ;

    CURSOR cur_stg (p_org_code IN VARCHAR2 DEFAULT NULL)
    IS
...
BEGIN
    ...
    LOOP
        ...
        l_item_int_tbl (i).item_catalog_group_name :=
        ...
        ELSIF UPPER (r.item_type) = 'ASSET ITEM'
        THEN
            l_item_int_tbl (i).template_name := 'Fixed Asset';
        END IF;
