CREATE OR REPLACE FORCE VIEW IFSAPP.SBL_SMS_WS_RETURN
...
SELECT w.rma_no,
     ...
     WHERE w.site IN
           ('JWSS', 'SAOS', 'SWSS', 'WSMO', 'SITM', 'WITM', 'SSAM')
       AND w.sales_price < 0
       AND w.order_no IS NOT NULL
       AND w.line_no IS NOT NULL
       AND w.rel_no IS NOT NULL
       AND TO_CHAR (w.sales_date, 'MM/DD/YYYY') =
           TO_CHAR (SYSDATE, 'MM/DD/YYYY');


GRANT SELECT ON IFSAPP.SBL_SMS_WS_RETURN TO SMS;
