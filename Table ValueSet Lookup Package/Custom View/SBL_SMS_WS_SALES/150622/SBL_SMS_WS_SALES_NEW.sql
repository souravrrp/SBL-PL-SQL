/* Formatted on 6/15/2022 3:55:00 PM (QP5 v5.381) */
...
AS
      SELECT v.trip_no,
             ...
             ifsapp.customer_info_api.get_name (
                 ifsapp.hpnret_customer_order_api.get_customer_no (
                     v.oe_order_no))
                 customer_name,
             (SELECT c.VALUE
               ...
               WHERE     EXISTS
                             (SELECT 1
                               ...
                               WHERE     cot.order_no = v.oe_order_no
                                     AND cot.customer_no = c.supplier_id)
                     AND c.method_id_db = 'PHONE')
                 phone_no, --c.supplier_id = ifsapp.customer_order_api.get_customer_no (v.oe_order_no)
             MAX (v.rowversion)
                 delivery_date,
             v.vat_receipt_no
        FROM (SELECT t.trip_no,
               ...
               WHERE     EXISTS
                             (SELECT 1
                                FROM TRN_TRIP_PLAN_TAB
                               WHERE     trip_no = t.trip_no
                                     AND release_no = t.release_no
                                     AND ROWSTATE IN ('Delivered', 'Closed'))
                     --AND ifsapp.trn_trip_plan_api.get_status(t.trip_no, t.release_no) IN ('Delivered', 'Closed')
                     AND TO_CHAR (t.ROWVERSION, 'MM/DD/RRRR') =
                         TO_CHAR (SYSDATE, 'MM/DD/RRRR')
                     AND l.customer_no IN ('JWSS',
                                           'SAOS',
                                           'SWSS',
                                           'WSMO',
                                           'WITM',
