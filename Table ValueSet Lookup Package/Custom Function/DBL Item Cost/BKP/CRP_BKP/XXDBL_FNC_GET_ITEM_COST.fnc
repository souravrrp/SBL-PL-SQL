...
BEGIN
   ...
   -- LAST UPDATE DATE :18-DEC-2017   -- PURPOSE : CALCULATING ITEM COST BASED ON ORGANIZATION AND PERIOD
      ...
      SELECT COST
        ...
        FROM (SELECT COUNT (ccd.inventory_item_id) inventory_item_id,
               ...
               WHERE ccd.period_id = ccm.period_id
                     AND gp.calendar_code =
                           ('DBLCOSTCAL'
                            || TO_CHAR (TO_DATE (P_PERIOD, 'MON-YY'), 'YYYY'))
   ...
   THEN
      ...
      THEN
         SELECT SUM (
                   rt.po_unit_price * RT.QUANTITY * currency_conversion_rate)
                / SUM (RT.QUANTITY)
           INTO v_txn_cst
          ...
          WHERE     rt.po_header_id = ph.po_header_id
                ...
                AND TO_CHAR (TO_DATE (TRUNC (rt.transaction_date)), 'MON-RR') =
                      P_PERIOD;
      END IF;
