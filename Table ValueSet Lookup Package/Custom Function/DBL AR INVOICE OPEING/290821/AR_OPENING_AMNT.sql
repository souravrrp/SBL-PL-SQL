/* Formatted on 8/29/2021 4:24:50 PM (QP5 v5.354) */
SET DEFINE OFF;

CREATE OR REPLACE FUNCTION APPS.XXDBL_GET_AR_OPEN_BAL (
    P_LEDGER_ID     NUMBER,
    P_ORG_ID        NUMBER,
...
IS
    ...
    CURSOR P_OPENING_AMOUNT IS
        SELECT                                  
               SUM (BAL_ACCTD_TOTAL)     OPENING_AMOUNT
          FROM (WITH
                    AR_INVOICE
                    AS
                        (SELECT DISTINCT
                  ...
                  SELECT CUSTOMER_NUMBER,
                         ...
                         NVL (
                             SUM (
                                 ABS (
                                     ...
                                     + LEAST (NVL (ADJUST_ACCTD_AMOUNT_OA, 0),
                                              0))),
                             0)                                       CM_ACCTD_AMOUNT,
                         NVL (
                             SUM (
                         ...
                         NVL (
                             ...
                             0)                                       ADJUST_ACCTD_AMOUNT,
                         SUM (
                             (  ACCTD_AMOUNT
                              + NVL (CM_ACCTD_AMOUNT_OA, 0)
                              + NVL (BR_ACCTD_AMOUNT_OA, 0)
                              + NVL (ADJUST_ACCTD_AMOUNT_OA, 0)
                              - NVL (RECEIPT_ACCTD_AMOUNT_OA, 0)))    BAL_ACCTD_TOTAL
                    FROM (SELECT AR.CUSTOMER_NUMBER,
                                 ...
                                 APPS.XX_AR_PKG.GET_ADJ_AMOUNT (
                                     ...
                                     'ACCTD',
                                     P_DATE_FROM,
                                     P_DATE_TO)    ADJUST_ACCTD_AMOUNT_OA,
                                 APPS.XX_AR_PKG.GET_RECEIPT_AMOUNT (
                                     ...
                                     'ACCTD',
                                     P_DATE_FROM,
                                     P_DATE_TO)    RECEIPT_ACCTD_AMOUNT_OA,
