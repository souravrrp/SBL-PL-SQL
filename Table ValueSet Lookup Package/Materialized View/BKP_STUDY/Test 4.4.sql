/* Formatted on 6/19/2021 10:51:46 AM (QP5 v5.287) */
CREATE OR REPLACE FORCE VIEW APPS.XXDBL_INV_CON_RPT_MVIEW#
(
   SET_OF_BOOKS_ID,
   ORGANIZATION_ID,
   ORGANIZATION_NAME,
     ...
     FROM (SELECT '1.MOVE_ORDER' AS "TR_SOURCE",
           ...
           SELECT '3.EXP_PO' AS "TR_SOURCE",
            ...
            WHERE     "PD"."PO_DISTRIBUTION_ID" = "RT"."PO_DISTRIBUTION_ID"
                  ...
                  AND "PD"."DESTINATION_ORGANIZATION_ID" =
                         "C"."ORGANIZATION_ID"
...
--------------------------------------------------------------------------------
exec ad_zd_mview.upgrade('APPS','XXDBL_INV_CON_RPT_MVIEW');

SELECT * FROM dba_objects
WHERE object_name LIKE 'XXDBL_INV_CON_RPT_MVIEW%';


--------------------------------------------------------------------------------

/* Formatted on 6/19/2021 12:04:29 PM (QP5 v5.287) */
EXECUTE dbms_mview.refresh('APPS.XXDBL_INV_CON_RPT_MVIEW',method =>'C',ATOMIC_REFRESH=>FALSE);

BEGIN

   dbms_mview.refresh('APPS.XXDBL_INV_CON_RPT_MVIEW', method => 'C', atomic_refresh => FALSE);

END;

EXECUTE dbms_mview.refresh('APPS.XXDBL_INV_CON_RPT_MVIEW','C', NULL, TRUE,FALSE,1,0,0,FALSE,FALSE,TRUE);


--------------------------------------------------------------------------------

/* Formatted on 6/20/2021 9:23:40 AM (QP5 v5.287) */
BEGIN
   SYS.DBMS_SCHEDULER.create_job (
      job_name          => 'XXDBL_MVIEW_SCHEDULER',
      job_type          => 'PLSQL_BLOCK',
      job_action        => 'BEGIN EXECUTE dbms_mview.refresh(''APPS.XXDBL_INV_CON_RPT_MVIEW'',method => ''C'',ATOMIC_REFRESH=>FALSE) END;',
...
SELECT *
  FROM APPS.XXDBL_INV_CON_RPT_MVIEW
 WHERE     1 = 1
