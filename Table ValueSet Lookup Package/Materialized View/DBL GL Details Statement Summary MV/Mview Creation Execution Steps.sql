/* Formatted on 8/3/2021 4:39:28 PM (QP5 v5.287) */
...
SELECT *
  FROM dba_objects
 WHERE object_name LIKE 'XXDBL_GL_DTL_STAT_SUM_MV%';


--------------------------------------------------------------------------------


EXECUTE dbms_mview.refresh('APPS.XXDBL_GL_DTL_STAT_SUM_MV',method =>'C',ATOMIC_REFRESH=>FALSE);
...
EXECUTE dbms_mview.refresh('APPS.XXDBL_GL_DTL_STAT_SUM_MV','C', NULL, TRUE,FALSE,1,0,0,FALSE,FALSE,TRUE);
...
BEGIN
   SYS.DBMS_SCHEDULER.create_job (
      ...
      job_action        => 'BEGIN EXECUTE dbms_mview.refresh(''APPS.XXDBL_GL_DTL_STAT_SUM_MV'',method => ''C'',ATOMIC_REFRESH=>FALSE) END;',
...
/
 ...
 EXECUTE SYS.DBMS_SYNC_REFRESH.REGISTER_MVIEWS('XXDBL_GL_DTL_STAT_SUM_MV');
...
;
 --------------------------------------------------------------------------
      ----------XXDBL_GL_DTL_STAT_SUM_MV MVIEW DATALOAD--------------------------
      --------------------------------------------------------------------------

BEGIN
   IF p_mview = 'XXDBL_GL_DTL_STAT_SUM_MV'
   THEN
      BEGIN
