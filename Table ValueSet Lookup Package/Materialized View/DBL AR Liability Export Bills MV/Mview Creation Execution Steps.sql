...
CREATE OR REPLACE FORCE VIEW APPS.XXDBL_AR_LIAB_EXP_BILL_MV#
(
    LEDGER_ID,
   LEGAL_ENTITY_NAME,
   NAME,
...
AS
   SELECT CLM.LEDGER_ID,
       ...
       CASE
          ...
          ELSE
             (CASE
                 WHEN ABS (NVL (ABS (LOAN_BDT), 0) - NVL (ADJUSTMENT_BDT, 0)) =
                         0
                 THEN
                    NULL
                 ELSE
                    ABS (NVL (ABS (LOAN_BDT), 0) - NVL (ADJUSTMENT_BDT, 0))
              END)
       END
          BAL_DUE_BDT,
       CASE
          WHEN (CASE
                   WHEN ABS (NVL (ABS (LOAN_USD), 0) - NVL (ADJUST_USD, 0)) =
                           0
                   THEN
                      NULL
                   ELSE
  ...
  FROM (  SELECT NAME,
            ...
            FROM (  SELECT HOU.NAME,
                           ...
                           SUM (
                              NVL (CT.BR_AMOUNT, 1) * NVL (CT.EXCHANGE_RATE, 1))
                              BDT_AMOUNT
       ...
       (  SELECT                                                           --*
        ...
        SELECT CTR.ORG_ID,                                                 --*
               ...
               TO_DATE (CTR.ATTRIBUTE3, 'YYYY/MM/DD HH24:MI:SS') RECEIPT_DATE,
         ...
         WHERE     CTR.BR_AMOUNT IS NOT NULL
               ...
               AND TRUNC (TO_DATE (CTR.ATTRIBUTE3, 'YYYY/MM/DD HH24:MI:SS')) <=
                      '30-SEP-17') AD,
       (  SELECT ORG_ID,
                 TO_CHAR (
                    LISTAGG (TRX_NUMBER, ',')
  ...
  FROM (SELECT HOU.NAME,                                                   --*
         ...
         WHERE     CRA.CASH_RECEIPT_ID = MDIST.CASH_RECEIPT_ID
               ...
               AND NOT EXISTS
                      (SELECT 1
                         FROM APPS.RA_CUSTOMER_TRX_ALL CT
                        WHERE     CT.CUSTOMER_REFERENCE = CRA.RECEIPT_NUMBER
       ...
       (SELECT HOU.NAME,
         ...
         WHERE     LACT.CUSTOMER_TRX_ID = LADIST.CUSTOMER_TRX_ID
               AND LADIST.ACCOUNT_CLASS = 'REV'
               AND NVL (COMPLETE_FLAG, 'N') = 'Y'
               AND LADIST.CODE_COMBINATION_ID = LAGCC.CODE_COMBINATION_ID(+)
               AND LAGCC.SEGMENT5 = '322103'
...
SELECT *
  FROM dba_objects
 WHERE object_name LIKE 'XXDBL_AR_LIAB_EXP_BILL_MV%';


--------------------------------------------------------------------------------


EXECUTE dbms_mview.refresh('APPS.XXDBL_AR_LIAB_EXP_BILL_MV',method =>'C',ATOMIC_REFRESH=>FALSE);

EXECUTE DBMS_MVIEW.REFRESH_ALL_MVIEWS(failures,'C','', TRUE, FALSE, FALSE);

BEGIN
   DBMS_MVIEW.refresh ('APPS.XXDBL_AR_LIAB_EXP_BILL_MV',
                       method           => 'C',
                       atomic_refresh   => FALSE);
END;

EXECUTE dbms_mview.refresh('APPS.XXDBL_AR_LIAB_EXP_BILL_MV','C', NULL, TRUE,FALSE,1,0,0,FALSE,FALSE,TRUE);
...
BEGIN
   SYS.DBMS_SCHEDULER.create_job (
      ...
      job_action        => 'BEGIN EXECUTE dbms_mview.refresh(''APPS.XXDBL_AR_LIAB_EXP_BILL_MV'',method => ''C'',ATOMIC_REFRESH=>FALSE) END;',
...
/
 ...
 EXECUTE SYS.DBMS_SYNC_REFRESH.REGISTER_MVIEWS('XXDBL_AR_LIAB_EXP_BILL_MV');
...
;
 --------------------------------------------------------------------------
      ----------XXDBL_AR_LIAB_EXP_BILL_MV MVIEW DATALOAD--------------------------
      --------------------------------------------------------------------------

BEGIN
   ELSIF p_mview = 'XXDBL_AR_LIAB_EXP_BILL_MV'
   THEN
      BEGIN
         x_mview_name := 'XXDBL_AR_LIAB_EXP_BILL_MV';
         DBMS_MVIEW.REFRESH ('APPS.XXDBL_AR_LIAB_EXP_BILL_MV',
                             METHOD           => 'C',
                             ATOMIC_REFRESH   => FALSE);
         COMMIT;
